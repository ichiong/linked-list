...
(defn count-cons
  ...
  [xx]
  (count (cons-to-list xx)))

(defn delete-cons
  "Delete cons `elt` from `xx`."
  [elt xx]
    (cond (empty? xx) nil
          (= elt (:car xx)) (:cdr xx)
          :otherwise (Cons. (:car xx) (delete-cons elt (:cdr xx)))))

(defn delete
  "Delete `elt` from `xx`."
  [elt {:keys [data size]}]
    (List. (delete-cons elt data) (count-cons (delete-cons elt data))))
...
(defn delete-all-cons
  "Delete all cons occurrences of `elt` from `xx`."
  [elt xx]
     (cond (empty? xx) nil
           (= elt (:car xx)) (delete-all-cons elt (:cdr xx))
           :otherwise (Cons. (:car xx) (delete-all-cons elt (:cdr xx)))))

(defn delete-all
  "Delete all occurences of `elt` from `xx`."
  [elt {:keys [data size]}]
    (List. (delete-all-cons elt data) (count-cons (delete-all-cons elt data))))